...
class RIR_by_Foot(ctk.CTk):
    ...
    def download_and_apply_update(self, download_url, remote_version):
        ...
        try:
            ...
            bat_lines = [
                ...
                f'move /Y "{temp_download}" "{exe_path}" >nul',
                "if errorlevel 1 (",
                f'  copy /Y "{temp_download}" "{exe_path}" >nul',
                ")",
                f'start "" "{exe_path}"',
            ]
    ...
    def _compute_mining_next_unlock(self, state):
        ...
        mining = skills.get('1003', {}) if isinstance(skills, dict) else {}
        try:
            level = int(mining.get('boostedLevel', mining.get('level', 1)) or 1)
        except Exception:
            level = 1
            ...
            if diff_list:
                ...
                if self.no_change_save_count >= 5 and not self.drift_alert_active:
                    ...
                    if self.config.get("webhook_enabled"):
                        self.send_webhook("⚠️ Save Drift", msg)
    ...
    def ensure_profile_selected(self):
        ...
        if len(profiles) == 1:
            ...
            return True
        return self.show_profile_picker(profiles)
    def send_webhook(self, t, m):
        url = self.config.get("webhook_url", "").strip()
        if not url or not self.config.get("webhook_enabled"): return
        def _f():
            try:
                ctx = ssl.create_default_context(); ctx.check_hostname, ctx.verify_mode = False, ssl.CERT_NONE
                d = {"embeds": [{"title": t, "description": m, "color": 3915872, "timestamp": time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime())}]}
                req = urllib.request.Request(url, data=json.dumps(d).encode('utf-8'), headers={'Content-Type': 'application/json', 'User-Agent': 'Mozilla/5.0'})
    ...
    def send_test_webhook(self):
        self.send_webhook("🔔 Discord Test", "Rocky Idle Reader webhook is connected.")
